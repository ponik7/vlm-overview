...
class CaptionGenerator(BaseModel):
    ...
    def build_rnn(self):
        ...
        lstm = tf.compat.v1.nn.rnn_cell.LSTMCell(
            ...
            initializer = self.nn.fc_kernel_initializer)
        if self.is_train:
            lstm = tf.compat.v1.nn.rnn_cell.DropoutWrapper(
                lstm,
    ...
    def build_optimizer(self):
        ...
        with tf.variable_scope('optimizer', reuse = tf.AUTO_REUSE):
            ...
            # Compute gradients
            gradients = tf.gradients(self.total_loss, tf.trainable_variables())
            ...
            if learning_rate_decay_fn is not None:
                learning_rate = learning_rate_decay_fn(learning_rate, self.global_step)
